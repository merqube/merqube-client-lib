...
#   timestamp: 2023-09-28T23:54:38+00:00
...
class MetricsArr(RootModel[List[MetricsArrItem]]):
...
class ClientMultiEBPortInitial(ClientMultiEBPortUpdate, ClientBaseVal):
    pass
...
class ClientBufferSpecific(BaseModel):
    ...
    buffer_end: Any = Field(..., description="eg the 30 in the above example. Must be < buffer_start.", example=0.7)
    excess_strategy: ExcessStrategy = Field(
        ExcessStrategy.buy_call, description="strategy for unused cash", example="keep_in_cash"
    )
    fee: Any = Field(0.0, description="expressed as a percentage of the transaction cost", example=0.05)
...
class HolidayCalendarSpec(RootModel[Union[FlatCalendarSchema, NestedCalendarSchema]]):
    root: Union[FlatCalendarSchema, NestedCalendarSchema]


class ArgumentsOrEnvVars(RootModel[List[List[StrictStr]]]):
...
class FsymEquityPosition(BasketPosition):
    model_config = ConfigDict(
        extra="forbid",
    )
...
class IntradayPublishConfigTarget(
    RootModel[
        Union[
...
class IntradayPublishConfig(
    RootModel[Optional[Dict[str, Union[IntradayPublishConfigItem, List[IntradayPublishConfigTarget]]]]]
):
...
class ClientMultiEBConfig(
    ClientMultiEBPortInitial, ClientMultiEBSpecific, ClientRuntimeInfo, ClientEquityBasketConfigBase
):
    pass
    model_config = ConfigDict(
        extra="forbid",
