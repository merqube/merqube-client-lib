...
#   timestamp: 2023-08-09T00:09:30+00:00
...
class IndexBenchmark(BaseModel):
    ...
    ticker: StrictStr = Field(..., description="accepts either FSYM_ID, or MQI:XXX to benchmark against another index.")
    threshold: float
...
class ClientSSTRSpecific(BaseModel):
    ...
    reinvestment_type: ReinvestmentType = Field(
        ReinvestmentType.AT_OPEN, description="set to the type of reinvestment to apply", example="AT_OPEN"
    )
...
class ClientMultiEBSpecific(BaseModel):
    ...
    corporate_actions: CorporateActions = Field(
        default_factory=lambda: CorporateActions.parse_obj({"reinvest_dividends": True}),
        description="how to handle corporate actions",
        example={"reinvest_dividends": True},
    )
    level_overrides_csv_path: Optional[StrictStr] = Field(
        None,
...
class CalendarIdentifiers(BaseModel):
    calendar_identifiers: Optional[List[constr(regex=r"^(FUT|MIC|FX|SM|MQI):.+$", strict=True)]] = None
...
class HolidayCalendarSpec(BaseModel):
    __root__: Union[FlatCalendarSchema, NestedCalendarSchema]
...
class ClientIndexConfigBase(BaseModel):
    ...
    base_date: constr(regex=r"^[0-9]{4}[-/][0-9]{2}[-/][0-9]{2}", strict=True) = Field(..., example="2000-01-04")
...
class ClientMultiEBConfig(ClientIndexConfigBase, ClientMultiEBSpecific):
    pass
...
class IndexDefinitionPost(BaseModel):
    ...
    base_date: Optional[constr(regex=r"^[0-9]{4}[-/][0-9]{2}[-/][0-9]{2}", strict=True)] = Field(
        None, example="2000-01-04"
    )
...
NestedCalendarSchema.update_forward_refs()
